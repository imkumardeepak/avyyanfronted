...
export const createTaskColumns = (
  ...
  },
  {
    accessorKey: 'status',
    header: 'Status',
    cell: ({ row }: { row: { getValue: (key: string) => unknown; original: Task } }) => {
      const status = (row.getValue('status') as Task['status']).toUpperCase();
      return <Badge className={getStatusColor(status)}>{status.replace('-', ' ')}</Badge>;
    },
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const task = row.original;
      return <TaskActions task={task} onEdit={onEdit} onDelete={onDelete} onView={onView} />;
    },
  },
];
