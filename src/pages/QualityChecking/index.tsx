...
const QualityChecking: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rollConfirmations = await RollConfirmationService.getRollConfirmationsByAllotId(formData.allotId);
      ...
      if (!existingRoll) {
        toast.error('Error', 'Roll not found. Please capture the roll first in Production Confirmation.');
        setIsLoading(false);
        return;
      }
    ...
    <div className="p-2 max-w-4xl mx-auto">
      <Card className="shadow-md border-0">
        <CardHeader className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-t-lg py-3">
          <CardTitle className="text-white text-base font-semibold text-center">Quality Checking</CardTitle>
        </CardHeader>
        ...
        <CardContent className="p-3">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-center pt-1">
              <Button type="submit" disabled={isLoading} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-1.5 h-8 min-w-28">
                {isLoading ? (
