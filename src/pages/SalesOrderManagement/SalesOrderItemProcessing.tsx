...
// Interfaces
interface LocationState { orderData?: SalesOrderDto; selectedItem?: SalesOrderItemDto; }
interface ProductionCalculation { needle: number; feeder: number; rpm: number; constant: number; stichLength: number; count: number; efficiency: number; productionPerDay: number; productionPerHour: number; productionPerMinute: number; }
interface MachineSelection { selectedMachine: MachineResponseDto | null; }
interface MachineLoadDistributionItem { machineId: number; machineName: string; allocatedRolls: number; allocatedWeight: number; estimatedProductionTime: number; isEditing?: boolean; customParameters?: { needle: number; feeder: number; rpm: number; efficiency: number; constant: number; }; }
interface RollInput { actualQuantity: number; rollPerKg: number; }
interface RollCalculationData extends RollInput { numberOfRolls: number; totalWholeRolls: number; fractionalRoll: number; fractionalWeight: number; }
interface AdditionalFields { yarnLotNo: string; counter: string; colourCode: string; reqGreyGsm: number | null; reqGreyWidth: number | null; reqFinishGsm: number | null; reqFinishWidth: number | null; }
interface PackagingDetailsState { coreType: 'with' | 'without'; tubeWeight: number; tapeColorId: number | null; }
...
const SalesOrderItemProcessingRefactored = () => {
  ...
  // State declarations
  const [selectedOrder, setSelectedOrder] = useState<SalesOrderDto | null>(locationState?.orderData || null);
  const [selectedItem, setSelectedItem] = useState<SalesOrderItemDto | null>(locationState?.selectedItem || null);
  ...
  const [packagingDetails, setPackagingDetails] = useState<PackagingDetailsState>({ coreType: 'with', tubeWeight: 1, tapeColorId: null });
  const [machineSelection, setMachineSelection] = useState<MachineSelection>({ selectedMachine: null });
  ...
  const [rollCalculation, setRollCalculation] = useState<RollCalculationData>({ actualQuantity: 0, rollPerKg: 0, numberOfRolls: 0, totalWholeRolls: 0, fractionalRoll: 0, fractionalWeight: 0 });
  const [productionCalc, setProductionCalc] = useState<ProductionCalculation>({ needle: 0, feeder: 0, rpm: 0, constant: 0.00085, stichLength: 0, count: 0, efficiency: 0, productionPerDay: 0, productionPerHour: 0, productionPerMinute: 0 });
  ...
  useEffect(() => {
    if (locationState?.orderData && locationState?.selectedItem) {
      ...
      setRollInput(prev => ({ ...prev, actualQuantity: parseFloat(locationState.selectedItem?.actualQty || '0') || prev.actualQuantity }));
  ...
  useEffect(() => {
    if (selectedItem) {
      setRollInput(prev => ({ ...prev, actualQuantity: parseFloat(selectedItem.actualQty || '0') || prev.actualQuantity }));
    }
  }, [selectedItem]);
  ...
  useEffect(() => {
    if (machines?.length) {
      ...
      setProductionCalc(prev => ({ ...prev, needle: defaultMachine.needle, feeder: defaultMachine.feeder, rpm: defaultMachine.rpm }));
    }
  }, [machines]);

  // Parse description values
  const parseDescriptionValues = (description: string) => {
    if (!description) return { stitchLength: 0, count: 0, weightPerRoll: 0, numberOfRolls: 0, diameter: 0, gauge: 0, composition: '' };

    const desc = description.toLowerCase();
    const values = { stitchLength: 0, count: 0, weightPerRoll: 0, numberOfRolls: 0, diameter: 0, gauge: 0, composition: '' };

    // Parse patterns
    const patterns = {
      stitchLength: [/s\.?l\.?\s*[:=]?\s*([0-9]+\.?[0-9]*)/i, /s\/l\s*[:=]?\s*([0-9]+\.?[0-9]*)/i, /stitch\s+length\s*[:=]?\s*([0-9]+\.?[0-9]*)/i, /sl\s*[:=]?\s*([0-9]+\.?[0-9]*)/i],
      count: [/count\s*[:=]?\s*([0-9]+\.?[0-9]*)/i, /cnt\s*[:=]?\s*([0-9]+\.?[0-9]*)/i, /([0-9]+\.?[0-9]*)\s*count/i, /([0-9]+\.?[0-9]*)\s*cnt/i],
      weightPerRoll: [/wt\.\/roll\s*[:=]?\s*([0-9]+\.?[0-9]*)\s*kg\/roll/i, /weight\s+per\s+roll\s*[:=]?\s*([0-9]+\.?[0-9]*)\s*kg/i, /([0-9]+\.?[0-9]*)\s*kg\/roll/i, /wt\s*[:=]?\s*([0-9]+\.?[0-9]*)\s*kg\/roll/i, /([0-9]+\.?[0-9]*)\s*kg\s*per\s*roll/i, /roll\s*[:=]?\s*([0-9]+\.?[0-9]*)\s*kg/i, /([0-9]+\.?[0-9]*)\s*kg\s*roll/i],
      numberOfRolls: [/no\s*of\s*rolls\s*[:=]?\s*([0-9]+)/i, /number\s*of\s*rolls\s*[:=]?\s*([0-9]+)/i, /rolls\s*[:=]?\s*([0-9]+)/i],
      diameterGauge: [/dia\s*x\s*gg\s*:\s*([0-9]+)"?\s*x\s*([0-9]+)/i, /diameter\s*x\s*gauge\s*:\s*([0-9]+)"?\s*x\s*([0-9]+)/i, /dia\s*:\s*([0-9]+)"?\s*\|\s*gg\s*:\s*([0-9]+)/i],
      ...
      for (const pattern of patternList) {
        ...
        if (match && match[1]) {
          if (key === 'diameterGauge' && match[1] && match[2]) {
            ...
            values[key as keyof Omit<typeof values, 'diameter' | 'gauge' | 'composition'>] = parseFloat(match[1]);
          }
          break;
        }
      }
  ...
  const parsedDescriptionValues = useMemo(() => 
    selectedItem?.descriptions ? parseDescriptionValues(selectedItem.descriptions) : { stitchLength: 0, count: 0, weightPerRoll: 0, numberOfRolls: 0, diameter: 0, gauge: 0, composition: '' },
  [selectedItem?.descriptions]);

  // Set values from description
  useEffect(() => {
    if (selectedItem?.descriptions) {
      setProductionCalc(prev => ({ ...prev, stichLength: parsedDescriptionValues.stitchLength || prev.stichLength, count: parsedDescriptionValues.count || prev.count }));
      ...
      if (parsedDescriptionValues.numberOfRolls > 0 && rollInput.rollPerKg === 0) {
        ...
        if (actualQuantity > 0) {
          setRollInput(prev => ({ ...prev, rollPerKg: actualQuantity / parsedDescriptionValues.numberOfRolls }));
        }
      }
    }
  ...
  useEffect(() => {
    calculateProduction();
  }, [productionCalc.needle, productionCalc.feeder, productionCalc.rpm, productionCalc.stichLength, productionCalc.count, productionCalc.efficiency]);

  const calculateProduction = () => {
    const { needle, feeder, rpm, constant, stichLength, count, efficiency } = productionCalc;
    
    if (needle <= 0 || feeder <= 0 || rpm <= 0 || stichLength <= 0 || count <= 0 || efficiency <= 0) {
      setProductionCalc(prev => ({ ...prev, productionPerDay: 0, productionPerHour: 0, productionPerMinute: 0 }));
      return;
    }

    try {
      const efficiencyDecimal = efficiency / 100;
      const productionGramsPerMinute = (needle * feeder * rpm * stichLength * constant * efficiencyDecimal) / count;
  ...
  useEffect(() => {
    ...
    if (actualQuantity > 0 && rollPerKg > 0) {
      ...
      setRollCalculation({
        actualQuantity, rollPerKg: calculatedRollPerKg,
        numberOfRolls: parsedDescriptionValues.numberOfRolls,
        totalWholeRolls: parsedDescriptionValues.numberOfRolls,
        fractionalRoll: 0, fractionalWeight: 0
      });
      setRollInput(prev => ({ ...prev, rollPerKg: calculatedRollPerKg }));
    } else {
      setRollCalculation({ actualQuantity, rollPerKg, numberOfRolls: 0, totalWholeRolls: 0, fractionalRoll: 0, fractionalWeight: 0 });
    }
  ...
  const addMachineToDistribution = (machine: MachineResponseDto) => {
    ...
    if (fabricStructures && selectedItem) {
      let itemFabricType = selectedItem.descriptions ? extractFabricTypeFromDescription(selectedItem.descriptions) : '';
      if (!itemFabricType && selectedItem.stockItemName) itemFabricType = selectedItem.stockItemName.toLowerCase();
      
      const matchingFabric = fabricStructures.find(f => 
        itemFabricType.includes(f.fabricstr.toLowerCase()) || f.fabricstr.toLowerCase().includes(itemFabricType.split(' ')[0])
      );
    ...
    setSelectedMachines(prev => [...prev, {
      ...
      allocatedRolls: 0, allocatedWeight: 0, estimatedProductionTime: 0, isEditing: false,
      customParameters: { needle: machine.needle, feeder: machine.feeder, rpm: machine.rpm, efficiency: fabricEfficiency, constant: machine.constat || 0.00085 }
    }]);
  };
  ...
  const toggleMachineEdit = (machineId: number) => {
    setSelectedMachines(prev => prev.map(m => m.machineId === machineId ? { ...m, isEditing: !m.isEditing } : m));
  };

  const updateMachineParameters = (machineId: number, parameters: Partial<MachineLoadDistributionItem['customParameters']>) => {
    setSelectedMachines(prev => prev.map(m => 
      m.machineId === machineId ? { ...m, customParameters: { ...m.customParameters!, ...parameters } } : m
    ));
  };
  ...
  const saveMachineParameters = (machineId: number) => {
    setSelectedMachines(prev => prev.map(machine => {
      if (machine.machineId === machineId) {
        ...
        if (machine.allocatedWeight > 0 && productionCalc.stichLength > 0 && productionCalc.count > 0) {
          const efficiencyDecimal = params.efficiency / 100;
          const productionGramsPerMinute = (params.needle * params.feeder * params.rpm * productionCalc.stichLength * params.constant * efficiencyDecimal) / productionCalc.count;
  ...
  const updateMachineAllocation = (machineId: number, allocatedWeight: number) => {
    const actualQuantity = Number(rollInput.actualQuantity) || 0;
    const currentTotalAllocated = selectedMachines.reduce((sum, m) => m.machineId === machineId ? sum : sum + m.allocatedWeight, 0);
      ...
      if (machine.machineId === machineId) {
        ...
        if (allocatedWeight > 0) {
          ...
          if (params && productionCalc.stichLength > 0 && productionCalc.count > 0) {
            ...
            if (selectedMachineData && productionCalc.stichLength > 0 && productionCalc.count > 0) {
              let fabricEfficiency = productionCalc.efficiency;
              if (fabricStructures && selectedItem) {
                let itemFabricType = selectedItem.descriptions ? extractFabricTypeFromDescription(selectedItem.descriptions) : '';
                if (!itemFabricType && selectedItem.stockItemName) itemFabricType = selectedItem.stockItemName.toLowerCase();
              ...
              const productionGramsPerMinute = (selectedMachineData.needle * selectedMachineData.feeder * selectedMachineData.rpm * productionCalc.stichLength * constant * efficiencyDecimal) / productionCalc.count;
              const productionKgPerHour = (productionGramsPerMinute / 1000) * 60;
              if (productionKgPerHour > 0) estimatedTime = allocatedWeight / productionKgPerHour;
            }
          }
        ...
        return { ...machine, allocatedRolls: rolls, allocatedWeight, estimatedProductionTime: estimatedTime };
      }
      return machine;
    }));
  };
  ...
  const autoDistributeLoad = () => {
    if (selectedMachines.length === 0) {
      alert('Please select at least one machine first.'); return;
    }
    ...
    if (!actualQuantity || actualQuantity <= 0) {
      alert('No quantity available for distribution.'); return;
    }
    if (!rollPerKg || rollPerKg <= 0) {
      alert('Please enter Roll per Kg value first.'); return;
    }
    ...
    let updatedMachines = selectedMachines.map((machine, index) => {
      ...
      if (fractionalRoll > 0 && index === selectedMachines.length - 1) machineRolls += fractionalRoll;
    ...
    updatedMachines = updatedMachines.map(machine => {
      ...
      if (selectedMachineData && machine.allocatedWeight > 0 && productionCalc.stichLength > 0 && productionCalc.count > 0) {
        let fabricEfficiency = productionCalc.efficiency;
        if (fabricStructures && selectedItem) {
          let itemFabricType = selectedItem.descriptions ? extractFabricTypeFromDescription(selectedItem.descriptions) : '';
          if (!itemFabricType && selectedItem.stockItemName) itemFabricType = selectedItem.stockItemName.toLowerCase();
  ...
  const handleMachineChange = (machineId: string) => {
    ...
    if (machine) {
      ...
      setProductionCalc(prev => ({ ...prev, needle: machine.needle, feeder: machine.feeder, rpm: machine.rpm }));
    }
  };
  ...
  const handleRefreshFromDescription = () => {
    if (selectedItem?.descriptions) {
      setProductionCalc(prev => ({ ...prev, stichLength: parsedDescriptionValues.stitchLength || 0, count: parsedDescriptionValues.count || 0 }));
  ...
  const handleAdditionalFieldChange = (field: keyof AdditionalFields, value: string | number | null) => {
    setAdditionalFields(prev => ({ ...prev, [field]: value }));
  };

  const handleCoreTypeChange = (coreType: 'with' | 'without') => {
    setPackagingDetails(prev => ({ ...prev, coreType, tubeWeight: coreType === 'without' ? 0 : (prev.tubeWeight === 0 ? 1 : prev.tubeWeight) }));
  };
    ...
    try {
      ...
      if (fabricStructures && fabricTypeFromDescription) {
        ...
        const matchingFabric = fabricStructures.find(f => 
          ...
          f.fabricstr.toLowerCase().includes(fabricTypeFromDescription.toLowerCase().split(' ')[0])
        );
      ...
      if (itemDescription.includes('honeycomb') || itemDescription.includes('honey comb')) twentyFirstChar = 'H';
  ...
  const handleProcessItem = async () => {
    ...
    if (selectedMachines.length === 0) {
      alert('Please select at least one machine before processing.'); return;
    }
    ...
    if (selectedMachines.length > 0 && totalAllocated > actualQuantity) {
      alert(`Error: Total allocated weight (${totalAllocated.toFixed(2)} kg) exceeds actual quantity (${actualQuantity.toFixed(2)} kg). Please adjust machine allocations before processing.`);
      return;
    }
    ...
    try {
      const machineAllocations = selectedMachines.map(machine => ({
        ...
        rollBreakdown: { wholeRolls: [], fractionalRoll: { quantity: 0, weightPerRoll: 0, totalWeight: machine.allocatedWeight } },
      ...
      const requestData = {
        ...
        reqGreyGsm: additionalFields.reqGreyGsm, reqGreyWidth: additionalFields.reqGreyWidth,
        reqFinishGsm: additionalFields.reqFinishGsm, reqFinishWidth: additionalFields.reqFinishWidth,
        partyName: selectedOrder.partyName,
        tubeWeight: packagingDetails.tubeWeight,
        tapeColor: packagingDetails.tapeColorId ? tapeColors.find(color => color.id === packagingDetails.tapeColorId)?.tapeColor || '' : '',
        machineAllocations,
      };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border rounded-xl bg-card p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="sm" onClick={handleGoBack} className="hover:bg-accent transition-colors">
            ...
            <div className="space-y-1">
              <div className="flex items-center space-x-3">
                ...
                <div className="flex items-center space-x-2">
                  <span className="font-medium text-primary bg-primary/10 px-2 py-0.5 rounded">{selectedItem?.stockItemName}</span>
                </div>
              </div>
      ...
      {currentPage === 1 && (
        <>
          ...
          <ProductionTimingCalculation
            machines={machines} isLoadingMachines={isLoadingMachines} selectedItem={selectedItem}
            parsedDescriptionValues={parsedDescriptionValues} extractFabricTypeFromDescription={extractFabricTypeFromDescription}
            fabricStructures={fabricStructures || []} productionCalc={productionCalc} machineSelection={machineSelection}
            onMachineChange={handleMachineChange} onProductionValueChange={handleProductionValueChange}
      ...
      {currentPage === 2 && (
        <>
          <RollCalculation
            rollInput={rollInput} rollCalculation={rollCalculation} selectedItem={selectedItem}
            parsedDescriptionValues={parsedDescriptionValues} onRollInputChange={handleRollInputChange}
          />
          <MachineLoadDistribution
            machines={machines} isLoadingMachines={isLoadingMachines} selectedMachines={selectedMachines}
            rollInput={rollInput} showMachineDistribution={showMachineDistribution}
            onToggleMachineDistribution={() => setShowMachineDistribution(!showMachineDistribution)}
            onAddMachineToDistribution={addMachineToDistribution} onRemoveMachineFromDistribution={removeMachineFromDistribution}
      ...
      {currentPage === 3 && (
        <PackagingDetails 
          rollPerKg={rollInput.rollPerKg} onCoreTypeChange={handleCoreTypeChange}
          onTubeWeightChange={handleTubeWeightChange} onTapeColorChange={handleTapeColorChange}
          tubeWeight={packagingDetails.tubeWeight} tapeColorId={packagingDetails.tapeColorId}
        />
      )}
      ...
      {currentPage === 4 && (
        <>
          <AdditionalInformation
            additionalFields={additionalFields} selectedOrder={selectedOrder}
            onAdditionalFieldChange={handleAdditionalFieldChange} count={productionCalc.count}
            rollPerKg={rollInput.rollPerKg} needle={productionCalc.needle}
            feeder={productionCalc.feeder} stichLength={productionCalc.stichLength}
          />
          <ProcessingActions
            selectedItem={selectedItem} selectedOrder={selectedOrder}
            isProcessing={isProcessing} isItemProcessing={isItemProcessing}
      ...
      <div className="flex justify-between mt-6">
        ...
        <Button variant="outline" onClick={() => setCurrentPage(prev => Math.min(prev + 1, 4))} disabled={currentPage === 4}>
          Next
        </Button>
      </div>
    </div>
